...
const Register = ({ closeModal }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            try {
                await api.post(`/v2/internship/apply`, {
                    name,
                    email,
            ...
            const pointsRes = await api.get("/v2/codeup-points");

            const userPoints = pointsRes.data.points?.[0]?.points || 0;
            const remainingAmount = PRICE - userPoints;

            // 🟢 STEP 3: Create Razorpay order
            const { data } = await api.post("/v2/academies/create-order", { amount: remainingAmount > 0 ? remainingAmount : PRICE });
            ...
            setErrorMessage(error?.response?.data?.message || "Enrollment failed. Please try again.");
            ...
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden w-full max-w-[650px] max-h-[95vh] overflow-y-auto">
                ...
                {mode === "success" && (
                    <div className="p-6 md:p-10 text-center">
                        ...
                        <button onClick={closeModal} className="bg-[#0b2239] text-white font-semibold py-3 px-8 rounded-xl hover:opacity-90 transition">
                            Back to Home
                        </button>
                    </div>
                )}
                ...
                {mode === "error" && (
                    <div className="p-6 md:p-10 text-center">
                        ...
                        <h2 className="text-xl font-bold text-gray-800 mb-8">{errorMessage}</h2>

                        <button onClick={closeModal} className="bg-[#0b2239] text-white font-semibold py-3 px-8 rounded-xl hover:opacity-90 transition">
                            {errorMessage?.toLowerCase().includes("already") ? "Back to Home" : "Try again later"}
                        </button>
                    </div>
                )}
                ...
                {mode === "form" && (
                    <>
                        ...
                        <div className="bg-[#0b2239] px-8 py-6 text-white">
                            <button onClick={closeModal} className="flex items-center gap-2 text-blue-200 text-sm mb-4 hover:text-white">
                                <ArrowLeft size={16} /> Back
                            </button>

                            <h2 className="text-xl font-bold">Registration for Summer Internship 2026</h2>

                            <p className="text-blue-200 text-sm mt-1">Fill your details and choose a course to enroll</p>
                        </div>
                        ...
                        <div className="p-8 space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    {courses.map((item) => {
                                        ...
                                        return (
                                            ...
                                            ${isSelected ? "border-[#0b2239] bg-[#0b2239]/5 shadow-lg" : "border-gray-200 hover:border-[#0b2239] hover:shadow-md"}`}
                                            >
                                                <input type="radio" name="course" value={item.id} checked={isSelected} onChange={() => setCourse(item.id)} className="mt-1 accent-[#0b2239]" />
                            ...
                            {/* MODE */}
                            <div>
                                <label className="text-sm font-semibold text-gray-800 mb-3 block">Mode of Learning *</label>

                                <div className="flex gap-6">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="mode" value="online" checked={courseMode === "online"} onChange={() => setCourseMode("online")} className="accent-[#0b2239]" />
                                        Online
                                    </label>

                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="mode" value="offline" checked={courseMode === "offline"} onChange={() => setCourseMode("offline")} className="accent-[#0b2239]" />
                                        Offline
                                    </label>
                                </div>

                                {errors.mode && <p className="text-red-500 text-xs mt-2">{errors.mode}</p>}
                            </div>
                            ...
                            <div>
                                ...
                                <input className={inputClass(errors.college)} placeholder="Your college / university name" value={college} onChange={(e) => setCollege(e.target.value)} />
                            ...
                            <div>
                                ...
                                <div className="flex items-center rounded-xl px-4 py-3 bg-white border border-gray-300 focus-within:border-codeup-blue">
                                    <span className="text-gray-500 mr-3 text-sm font-medium">+91</span>

                                    <input type="text" className="w-full outline-none text-sm text-gray-800 bg-transparent" placeholder="9876543210" maxLength={10} value={phone} onChange={(e) => setPhone(e.target.value.replace(/\D/g, ""))} />
                                </div>
                            ...
                            ${loading ? "bg-gray-400 cursor-not-allowed" : "bg-[#0b2239] hover:opacity-90 text-white"}`}
                            >
